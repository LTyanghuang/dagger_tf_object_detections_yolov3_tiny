...
names_dict = {}
cnt = 0
f = open('./data/widerface.names', 'r').readlines()
for line in f:
    line = line.strip()
    names_dict[line] = cnt
    cnt += 1
...
'''
voc_my_data = 'data/widerface_voc/train'
anno_path = [os.path.join(voc_my_data, 'Annotations')]
img_path = [os.path.join(voc_my_data, 'JPEGImages')]
trainval_path = [os.path.join(voc_my_data, 'ImageSets/Main/train.txt')]
test_path = [os.path.join(voc_my_data, 'ImageSets/Main/val.txt')]
...
def parse_xml(path):
    ...
    for obj in tree.findall('object'):
        ...
        # if int(xmax) - int(xmin) <= 2 or int(ymax) - int(ymin) <= 2:
        #     continue 
